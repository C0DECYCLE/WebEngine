...
window.addEventListener("compile", (_event: Event): void => {
    ...
    const camPosBuffer: SharedArrayBuffer = new SharedArrayBuffer(
        Float64Array.BYTES_PER_ELEMENT * 3
    );
    const camPos: Float64Array = new Float64Array(camPosBuffer);
    camPos[0] = 1_000_000;
    camPos[1] = 10;
    camPos[2] = 100_000;
    ...
    const positionsBuffer: SharedArrayBuffer = new SharedArrayBuffer(
        Float64Array.BYTES_PER_ELEMENT * 3 * n
    );
    const positions: Float64Array = new Float64Array(positionsBuffer);
    ...
    const matriciesBuffer: SharedArrayBuffer = new SharedArrayBuffer(
        Float64Array.BYTES_PER_ELEMENT * 16 * n
    );
    const matricies: Float32Array = new Float32Array(matriciesBuffer);
